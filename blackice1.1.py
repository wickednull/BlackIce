...
def build_menu_view(menu_items, page: int, compact: bool, reserved_rows: int = 14):
    """
    Returns (renderable_panel, index_map, total_pages).
    compact=True -> single column, paginated. compact=False -> Rich table with truncation.
    """
    ...
    if compact:
        ...
        for shown_idx, (real_idx, (label, _)) in enumerate(enumerate(menu_items[start:end], start=start+1), start=1):
            safe_label = truncate_label(label, max_label)
            lines.append(f"[cyan]{shown_idx}[/cyan]. {safe_label}")
            index_map[str(shown_idx)] = real_idx
        footer = f"Page {page}/{total_pages} • n=next, p=prev, q=quit • Choose number"
        panel = _Panel("\n".join(lines) if lines else "(no items)",
                       border_style="cyan",
                       title="BLACKICE COMMAND — Menu (Compact)",
                       subtitle=footer)
        return panel, index_map, total_pages
    else:
        table = _Table(title="BLACKICE COMMAND — Authorized Operations Console", show_lines=True, border_style="cyan")
        table.add_column("#", justify="right", no_wrap=True)
        max_label = max(20, width - 20)
        table.add_column("Action", no_wrap=True, overflow="fold")
        for shown_idx, (real_idx, (label, _)) in enumerate(enumerate(menu_items[start:end], start=start+1), start=1):
            safe_label = truncate_label(label, max_label)
            table.add_row(str(shown_idx), safe_label)
            index_map[str(shown_idx)] = real_idx
...
def main():
    ...
    with Live(layout, refresh_per_second=4, screen=False):
        ...
        while True:
            ...
            menu_panel, idx_map, total_pages = build_menu_view(MENU, page=page, compact=compact)

            layout["body"].update(menu_panel)
            layout["footer"].update(Panel(status_bar(), border_style="blue"))

            valid_choices = list(idx_map.keys())
            if page > 1:
                valid_choices += ["p"]
            if page < total_pages:
                valid_choices += ["n"]
            ...
            choice = Prompt.ask(
                ...
                default=next(iter(idx_map.keys())) if idx_map else ("n" if page < total_pages else "q")
            )
            ...
            if choice == "q":
                ...
                real_idx = idx_map[choice]  # 1-based index into MENU
                label, func = MENU[real_idx - 1]
                if label == "Exit":
                    console.print(Panel("Disconnecting from NETWATCH C.I.C… Link closed.", border_style="cyan"))
                    sys.exit(0)
                func()
