...
# ===== Responsive UI helpers (NEW) =====
import shutil as _shutil

def term_dims():
    """Return (width, height) using rich Console when possible, else fallback."""
    try:
        sz = console.size
        return sz.width, sz.height
    except Exception:
        sz = _shutil.get_terminal_size(fallback=(100, 28))
        return sz.columns, sz.lines

def truncate_label(text: str, maxw: int) -> str:
    """Truncate labels safely for narrow terminals."""
    if maxw <= 5:
        return text[:maxw]
    return text if len(text) <= maxw else (text[:maxw-1] + "…")

# >>> NEW: simple Enter pause used by main() <<<
def press_enter():
...
def build_menu_view(menu_items, page: int, compact: bool, reserved_rows: int = 14):
    """
    Returns (panel, page_items, total_pages).
    - page_items is a list of tuples: (global_index, label, func)
    - numbering on screen is always 1..N for the current page
    """
    from rich.table import Table as _Table
    from rich.panel import Panel as _Panel

    width, height = term_dims()
    usable_rows = max(8, height - reserved_rows)

    per_page = usable_rows
    ...
    # Build the visible slice and keep their *global* indices
    page_items = []
    for gidx in range(start, end):
        label, func = menu_items[gidx]
        page_items.append((gidx, label, func))

    if compact:
        lines = []
        max_label = max(20, width - 10)
        for i, (_gidx, label, _func) in enumerate(page_items, start=1):
            lines.append(f"[cyan]{i}[/cyan]. {truncate_label(label, max_label)}")
        footer = f"Page {page}/{total_pages} • n=next, p=prev, q=quit • Choose number"
        panel = _Panel("\n".join(lines) if lines else "(no items)",
                       border_style="cyan",
                       title="BLACKICE COMMAND — Menu (Compact)",
                       subtitle=footer)
        return panel, page_items, total_pages
    else:
        table = _Table(title="BLACKICE COMMAND — Authorized Operations Console",
                       show_lines=True, border_style="cyan")
        table.add_column("#", justify="right", no_wrap=True)
        max_label = max(20, width - 20)
        table.add_column("Action", no_wrap=True, overflow="fold")
        for i, (_gidx, label, _func) in enumerate(page_items, start=1):
            table.add_row(str(i), truncate_label(label, max_label))
        panel = _Panel(table, border_style="cyan",
                       subtitle=f"Page {page}/{total_pages} • n=next, p=prev, q=quit • Choose number")
        return panel, page_items, total_pages
...
def main():
    ...
    with Live(layout, refresh_per_second=4, screen=False):
        ...
        while True:
            term_w, _ = term_dims()
            compact = term_w < 110  # auto-compact on narrow terminals

            menu_panel, page_items, total_pages = build_menu_view(MENU, page=page, compact=compact)

            layout["body"].update(menu_panel)
            layout["footer"].update(Panel(status_bar(), border_style="blue"))

            # Build valid choices: 1..len(page_items) + n/p/q
            numeric_choices = [str(i) for i in range(1, len(page_items) + 1)]
            valid_choices = list(numeric_choices)
            if page > 1:
            ...
            default_choice = numeric_choices[0] if numeric_choices else ("n" if page < total_pages else "q")

            choice = Prompt.ask(
                "\nSelect option (number, n=next, p=prev, q=quit)",
            ...
            if choice == "q":
                ...
                # map page-local 1..N to global index directly
                idx_in_page = int(choice) - 1
                if idx_in_page < 0 or idx_in_page >= len(page_items):
                    continue
                gidx, label, func = page_items[idx_in_page]
                try:
                    if label == "Exit":
                        console.print(Panel("Disconnecting from NETWATCH C.I.C… Link closed.", border_style="cyan"))
                        sys.exit(0)
                    ...
                    console.print(Panel(f"[red]Module error:[/red] {e}", border_style="red"))
                press_enter()
